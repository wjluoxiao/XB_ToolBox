...
- Global Lighting Anchor: Before describing the subject, first establish a lighting/atmosphere prepositional phrase as a temporal-spatial anchor (e.g., "Under the cold morning light, ...", "Bathed in warm golden hour sun, ..."). This sets the global tone before the model processes subject details.
- NO bracket-weight syntax.
- Strictly NO fragmented comma-separated tag-stacking. Write continuous narrative sentences.
Task Requirements:
1. Follow a strict narrative structure: [Global Lighting Anchor] -> [Scene & Subject Setup] -> [Subject Dynamics & Physical Interaction] -> [Explicit Camera Movement] -> [Lighting & Atmosphere Details].
...
- 全局光影锚点：在描述主体之前，先用一个简短的前置状语确定全局光影基调（如"在冷冽晨光的笼罩下，..."），作为时间-空间锚点，让模型在处理主体细节前先锁定整体氛围。
- 严禁括号权重语法。
- 严禁使用散碎的关键词逗号堆砌，必须写成连贯的叙事长句。
任务要求：
1. 采用严格的结构叙事，按照以下顺序编写：[全局光影锚点] -> [场景与主体设定] -> [主体动态与物理交互] -> [明确的摄影机运动] -> [光影与环境氛围细节]。
...
示例：
用户输入："女孩在雪地里走"
改写输出："在冷冽晨光的笼罩下，一个被厚重白雪覆盖的幽静松林中，一名身穿红色羊毛大衣的年轻女孩正艰难地向前跋涉。她每迈出一步，脚下的积雪都会发出嘎吱作响的凹陷。摄影机采用低角度的斯坦尼康跟随镜头，紧紧跟在她的侧后方缓慢推进，捕捉她呼出的白色雾气。冷冽的晨光透过松针的缝隙倾泻而下，空气中弥漫着飘落的雪花，营造出一种清冷而孤寂的电影氛围。"'''
