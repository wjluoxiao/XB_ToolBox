...
def load_wav(wav, target_sr, min_sr=16000):
    # Use soundfile to load audio (avoids torchaudio's torchcodec requirement in newer versions)
    # This matches the official torchaudio.load(backend='soundfile') behavior
    audio_data, sample_rate = sf.read(wav, dtype='float32', always_2d=True)
    # soundfile returns (samples, channels), convert to torch tensor (channels, samples)
    speech = torch.from_numpy(audio_data.T)
    # Convert to mono by averaging channels (matches official: speech.mean(dim=0, keepdim=True))
