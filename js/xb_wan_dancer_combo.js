...
// ============================================================
// XB_WanDancerCombo — Wan Dancer 综合输入 (波形 + 拖拽切片)
// 基于 XB_AudioSlicerV1 的 JS，适配 WanDancer 节点
// ============================================================

const PAD = 10, HANDLE_HIT = 12;

app.registerExtension({
    name: "XB_ToolBox.WanDancerCombo",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "XB_WanDancerCombo") return;
        ...
        nodeType.prototype.onNodeCreated = function () {
            ...
            audioEl.addEventListener("loadedmetadata", () => { totalDur = audioEl.duration; const curStart = parseFloat(wStart?.value) || 0; const curEnd = parseFloat(wEnd?.value) || 0; if (curStart === 0 && (curEnd <= 0 || curEnd > totalDur + 1 || Math.abs(curEnd - 10.0) < 0.001)) { wStart.value = 0; wEnd.value = totalDur; } syncWidgets(); audioEl.currentTime = parseFloat(wStart?.value) || 0; });
