...
app.registerExtension({
    name: "XB_ToolBox.ModelLoaderV3_GGUF",
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (nodeData.name !== "XB_ModelLoaderV3_GGUF") return;
            ...
            const refreshLists = async (keyword) => {
                ...
                try {
                    ...
                    if (resp.ok) {
                        ...
                        if (data.clips) { wClip1.options.values = data.clips; wClip2.options.values = data.clips; if (!data.clips.includes(wClip1.value)) wClip1.value = data.clips[0]; if (!data.clips.includes(wClip2.value)) wClip2.value = data.clips[0]; }
                        if (data.vaes) { wVae1.options.values = data.vaes; wVae2.options.values = data.vaes; if (!data.vaes.includes(wVae1.value)) wVae1.value = data.vaes[0]; if (!data.vaes.includes(wVae2.value)) wVae2.value = data.vaes[0]; }
                        if (data.loras) { const lo = ["无", ...data.loras]; node._lora_slots.forEach(s => { s.lora.options.values = lo; }); }
